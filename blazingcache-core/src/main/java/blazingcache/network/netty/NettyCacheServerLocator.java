...
public class NettyCacheServerLocator extends GenericNettyBrokerLocator {
    ...
    public NettyCacheServerLocator(String host, int port, boolean ssl) {
        this.servers.add(new ServerHostData(host, port, "", ssl, new HashMap<String, String>()));
    }

    public NettyCacheServerLocator(final List<InetSocketAddress> servers, boolean ssl) {
        servers.forEach(s -> {
    ...
    public NettyCacheServerLocator(String host, int port, String host2, int port2) {
